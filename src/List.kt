...
fun comprimir(l: List): List = when(l){
	is Node -> Node(l.info, auxComprimir(l.prox, l.info))
	else -> Nil
}

fun auxComprimir(l1: List, i: Int): List = when{
	l1 is Node && l1.info == i -> auxComprimir(l1.prox, i)
	l1 is Node && l1.info != i -> Node(l1.info, auxComprimir(l1.prox, l1.info))
	else -> Nil
}
